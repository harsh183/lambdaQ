...
def duplicate_or_terminate(ms_time_spent)
  ...
  if ms_time_spent > target_time
    ...
    Thread.new do
      ...
      # the terminal will be closed if consumer_duplicate.rb is not running anymore.
      `#{'xterm -e ruby consumer_duplicate.rb'}`
    end
  end
  
  # consumer_progenitor will never die to prevent the situation where the last worker terminates itself.
end
